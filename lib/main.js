(function() {
  'use strict';
  var D3F, Effstring_error, Effstring_lib_syntax_error, Effstring_syntax_error, Effstring_validation_error, _default_locale, _fmtspec_re, _hint_as_locale_cfg, _locale_cfg_from_bcp47, _locale_cfg_from_hints, _to_width, debug, f, log, new_ftag, rpr, types, width_of,
  ...
  types = {
    ...
    validate: function(typename, x) {
      ...
      throw new Effstring_validation_error('Ωfstr___3', typename, x);
    },
    //---------------------------------------------------------------------------------------------------------
    isa: {
  ...
  //-----------------------------------------------------------------------------------------------------------
  _to_width = function(text, fmt_cfg) {
    var p;
    switch (fmt_cfg.align) {
      ...
      case '^':
        p = 0;
        while (true) {
          if (!((width_of(text)) > fmt_cfg.width)) {
            break;
          }
          p++;
          if ((modulo(p, 2)) === 0) {
      ...
      for (idx = i = 0, len = expressions.length; i < len; idx = ++i) {
        ...
        if (part.startsWith(':')) {
          if ((match = part.match(_fmtspec_re)) == null) {
            throw new Effstring_syntax_error('Ωfstr___5', part);
          }
          ...
          try {
            ...
            error = error1;
            throw new Effstring_lib_syntax_error('Ωfstr___6', fmt_spec, error);
          }
          if ((fmt_cfg = D3F.formatSpecifier(fmt_spec)).width != null) {
